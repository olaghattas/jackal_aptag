...
class Jackal:
	def __init__(self,prev_time = None, current_time= None):
		...
		self.prev_error = 0
		self.ap_id = [None]*1
		self.prev_time = prev_time if prev_time is not None else time.time()
	...
	def move_towards_tag(self):
		if self.pose_x is not None and self.pose_y is not None and self.pose_z is not None:
			...
			while dist_to_goal > 0.0:
				self.current_time = current_time if current_time is not None else time.time()
				current_error = self.ang_pose_z
				dt = self.current_time - self.prev_time 

				self.vel.linear.x = 1
				self.vel.angular.z = self.controller.velocity_control(current_error, dt, self.prev_error)
				self.pub.publish(self.vel)
